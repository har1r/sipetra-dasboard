import mongoose, { Schema, models, model, InferSchemaType,
  HydratedDocument, } from "mongoose";
import { TASK_STAGES, TASK_STATUS, TASK_SERVICE, SUBDISTRICT_DATA } from '@/constants/task';
...
const taskSchema = new Schema({
  ...
  baseData: {
    ...
    taxObjectSubdistrict: { type: String, enum: Object.keys(SUBDISTRICT_DATA) },
    taxObjectVillage: { type: String, enum: Object.values(SUBDISTRICT_DATA).flat() },
    landArea: { type: Number },
    buildingArea: { type: Number },
  },
  requestedData: {
    taxObjectAddress: { type: String, required: true, trim: true },
    taxObjectSubdistrict: { type: String, enum: Object.keys(SUBDISTRICT_DATA), required: true },
    taxObjectVillage: { type: String, enum: Object.values(SUBDISTRICT_DATA).flat(), required: true },
  },
  requestedChanges: [{
...
export type TaskDocument =
  HydratedDocument<TaskSchemaType>;
